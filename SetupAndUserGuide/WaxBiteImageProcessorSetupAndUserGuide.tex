...
Documents/
  GeckoData/              <- your storage folder (name it anything)
    Animals/
      animal1/
        img/              <- original images live here permanently
...
\begin{enumerate}[leftmargin=*, label=\textbf{\arabic*.}]
  \item \textbf{Copy images into the repository.}
    Copy all image files from \texttt{Animals/animal1/img/} into
...
Image file names must begin with a date in one of two formats, followed by any
additional identifier:

\begin{itemize}[leftmargin=*]
  \item \texttt{YYYY\_MM\_DD} (preferred) --- e.g.\ \texttt{2024\_03\_15\_LG282.jpg}
  \item \texttt{MM\_DD\_YYYY} (legacy) --- e.g.\ \texttt{03\_15\_2024\_LG282.jpg}
\end{itemize}

The application detects which format is used automatically. The date prefix is used
to sort images chronologically and to label results in plots. Images that do not
start with a date in one of these formats will cause errors.

\begin{tipbox}
\textbf{Recommendation:} Use the \texttt{YYYY\_MM\_DD} format for new data. Files
sort chronologically in your file browser, and all new data in the lab follows this
convention.
\end{tipbox}
...
If the code has been updated and new packages are required, run
\texttt{pip install -r requirements.txt} after activating the virtual environment.
There is no need to recreate the virtual environment itself.
\end{warnbox}

% -----------------------------------------------------------------------
\section{Updating the Application}

When the code has been updated, use \texttt{git pull} to download the latest version.
Open a terminal in the \texttt{Wax-Bite-Image-Processor} folder (the top-level
repository folder, \emph{not} the \texttt{TemplateMatching} subfolder) and run:

\begin{lstlisting}
git pull
\end{lstlisting}

Then navigate back into the \texttt{TemplateMatching} folder, activate the virtual
environment, and install any new packages:
...
The \texttt{pip install} step is only needed when the update includes new package
dependencies. Running it when no new packages are required is harmless --- it will
simply report that everything is already installed.
...
The default window width is controlled by \texttt{MAX\_WIDTH} in \texttt{utils.py} (default: 1200 pixels).
...
There are six types of markers you can place:
...
Center Tooth    & \texttt{3} & The central-most tooth (only one center allowed) \\
Center Gap      & \texttt{4} & The central-most gap (only one center allowed) \\
No Bite         & \texttt{5} & A position where the gecko did not bite into the wax \\
Center No Bite  & \texttt{6} & The central-most position, in a no-bite zone (only one center allowed) \\
\bottomrule
\end{tabular}
\end{center}

\begin{notebox}
\textbf{When to use No Bite:} Sometimes the gecko does not bite fully into the wax,
leaving a region with no visible tooth or gap imprints. Use the No Bite marker
(\texttt{5}) to indicate where teeth or gaps \emph{should} be but cannot be
determined from the wax impression. It is important to place the correct number of
No Bite markers in these zones so that teeth distal to the zone stay properly aligned
across time points. Compare with neighboring rows in the index or arclength plot to
verify the count. If the center of the jaw falls in a no-bite zone, use
Center No Bite (\texttt{6}) for that position.
\end{notebox}
...
\midrule
\texttt{1} / \texttt{2} / \texttt{3} / \texttt{4} / \texttt{5} / \texttt{6} & Switch mode (tooth / gap / center tooth / center gap / no bite / center no bite) \\
\texttt{Tab}        & Cycle through modes in order \\
\texttt{Space}      & Toggle boxes on/off (view image without overlays) \\
\texttt{S}          & Save and advance to the next image \\
\texttt{X}          & Save and stop (exit the manual loop entirely) \\
...
\begin{warnbox}
\textbf{Always save before closing} using \texttt{S}, \texttt{X}, or the arrow keys.
\texttt{Q} and \texttt{Esc} exit without saving the \emph{current} image, which will
cause \texttt{fitproj} to fail for that image if no previous annotation exists for it.
Images you navigated through using the arrow keys are saved automatically and are not
affected.
\end{warnbox}

\begin{tipbox}
\textbf{Stopping early:} If you only need to annotate a few images, press \texttt{X}
when you are done. This saves the current image and exits without opening the remaining
images. \texttt{S} and the arrow keys save but continue to the next image.
\end{tipbox}

\begin{tipbox}
\textbf{Window size:} The annotation window opens at the width set by \texttt{MAX\_WIDTH}
in \texttt{utils.py} (default: 1200 pixels), or the value passed via \texttt{--width} on
the command line. If the image appears too small, increase this value; if it doesn't fit
on your screen, decrease it. Avoid resizing the window manually by dragging its edges, as
this will cause click positions to be misaligned with markers.
\end{tipbox}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Left click} on a point: opens the projected 1D image for that date.
        Each tooth position is labeled with its index number (relative to the center
        marker, which is index~0).
  \item \textbf{Left click and drag} across a range of dates: opens the projected 1D
        ...
        the same width, with index labels on each strip. This is useful for visually
        comparing annotations across nearby days and identifying errors such as a tooth
        misidentified as a gap, or a missed gap causing all teeth distal to it to appear
        shifted relative to other days.
  \item \textbf{Right click} (ctrl+click on Mac): opens two windows side by side ---
        the manual annotation tool for that date, and a reference window showing the
        labeled 1D strips for that date and its two nearest neighbors. Both views show
        tooth index numbers, making it easy to identify a problematic tooth in the 1D
        reference and locate it in the 2D editor. The reference window closes
        automatically when you close the editor. Note: changes will not appear in the
        plots until you re-run \texttt{format} or \texttt{analyze}.
\end{itemize}
...
\begin{description}[leftmargin=0pt, style=nextline]
  ...
  \item[Image appears too small or too large]
    ...
    \texttt{python main.py manual --width 1400}. A good starting point is roughly half
    your screen width. Once you find a value that works, update \texttt{MAX\_WIDTH} in
    \texttt{utils.py} (currently 1200) to make it the default.
  ...
  \item[Plots are empty or show no data]
    ...
    or \texttt{analyze}. Also check that image filenames begin with a date in
    \texttt{YYYY\_MM\_DD} or \texttt{MM\_DD\_YYYY} format.
