...
from utils import CONFIG, Cross, Filter, Match, Tooth
...
class ImageProcessor:
    ...
    def __init__(self, file_names: list[str], index: int) -> None:
        ...
        # PATHS
        self._PATH_MATCHING = os.path.join(
            self._PATH_ROOT, "processed", "template matching", self.img_name
        )
        self._PATH_MANUAL = os.path.join(
            self._PATH_ROOT, "processed", "manual", self.img_name
        )
        self._PATH_FILTER = os.path.join(
            self._PATH_ROOT, "processed", "filter", self.img_name
        )
        self._PATH_FIT = os.path.join(
            self._PATH_ROOT, "processed", "fit", self.img_name
        )
        self._PATH_PROJECTION = os.path.join(
            self._PATH_ROOT, "processed", "projection", self.img_name
        )
        self._PATH_OUTPUT = os.path.join(
            self._PATH_ROOT, "processed", "output", self.img_name
        )
        ...
        assert os.path.isfile(os.path.join(self._PATH_IMG, self.file_name)), (
            f"'{self.file_name}' does not exist in img"
        )

        # image and image projection data
        image_proj_path = os.path.join(
            self._PATH_PROJECTION, f"projection{self.file_type}"
        )
        self.image = cv2.imread(os.path.join(self._PATH_IMG, self.file_name))
        self.image_proj = (
            cv2.imread(image_proj_path) if os.path.isfile(image_proj_path) else None
        )
        ...
        filtered_data_path = os.path.join(
            self._PATH_FILTER, "raw.csv"
        )  # all files are created at the same time

        self.matching_data = (
            pd.read_csv(matching_data_path)
            if os.path.isfile(matching_data_path)
            else None
        )
        self.manual_data = (
            pd.read_csv(manual_data_path) if os.path.isfile(manual_data_path) else None
        )
        self.manual_data_1D = (
            pd.read_csv(manual_data_path_1D)
            if os.path.isfile(manual_data_path_1D)
            else None
        )
        self.filtered_data = (
            pd.read_csv(filtered_data_path)
    ...
    def template_matching(self, display_time: bool = False) -> None:
        ...
        templates = [
            file
            for file in os.listdir(self._PATH_TEMPLATE)
        ...
        for template_path in templates:
            ...
            template = cv2.imread(
                os.path.join(template_dir, template_path), cv2.IMREAD_GRAYSCALE
            )
    ...
    def filter(self, display_time: bool = False) -> None:
        ...
        if CONFIG.FILTER == Filter.MANUAL:
            raw_data_path = os.path.join(self._PATH_MANUAL, "manual data.csv")
            assert os.path.isfile(raw_data_path), (
                f"'manual data.csv' does not exist in /processed/manual/{self.img_name} - did you run manual first?"
            )
        ...
        ax1.set_title(
            "Original", fontsize=10, fontweight="bold", fontname="Times New Roman"
        )
        ax2.set_title(
            "Gradient Raw", fontsize=10, fontweight="bold", fontname="Times New Roman"
        )
        ax3.set_title(
            "Smoothness Raw", fontsize=10, fontweight="bold", fontname="Times New Roman"
        )
        ax5.set_title(
            "Gradient Filtering",
            fontsize=10,
        ...
        ax6.set_title(
            f"Filtered Gradient: Threshold {CONFIG.GRAD_THRESHOLD}",
            fontsize=10,
            fontweight="bold",
            fontname="Times New Roman",
        )
        ax7.set_title(
            "Even Gradient Filtering",
            fontsize=10,
            fontweight="bold",
            fontname="Times New Roman",
        )
        ax8.set_title(
            f"Filtered Even Gradient: Threshold {CONFIG.GRAD_EVEN_THRESHOLD}",
            fontsize=10,
            fontweight="bold",
            fontname="Times New Roman",
        )
        ...
        ax10.set_title(
            f"Filtered Smoothness: Threshold {CONFIG.SMOOTH_THRESHOLD}",
            fontsize=10,
            fontweight="bold",
            fontname="Times New Roman",
        )
        ax11.set_title(
            "Even Smoothness Filtering",
            fontsize=10,
            fontweight="bold",
            fontname="Times New Roman",
        )
        ax12.set_title(
            f"Filtered Even Smoothness: Threshold {CONFIG.SMOOTH_EVEN_THRESHOLD}",
            fontsize=10,
            fontweight="bold",
            fontname="Times New Roman",
        )
        ...
        df_grad_even = self.filter_one(
            df_raw, CONFIG.GRAD_EVEN_THRESHOLD, "gradient_even"
        )
        ...
        df_smooth_even = self.filter_one(
            df_raw, CONFIG.SMOOTH_EVEN_THRESHOLD, "smoothness_even"
        )
        ...
        df_grad.to_csv("grad filtered.csv")
        df_grad_even.to_csv("grad even filtered.csv")
        df_smooth.to_csv("smooth filtered.csv")
        df_smooth_even.to_csv("smooth even filtered.csv")
        os.chdir(self._PATH_ROOT)
    ...
    def fit_project(self, display_time: bool = False):
        ...
        all_x = self.filtered_data["x"].to_numpy()
        all_y = self.filtered_data["y"].to_numpy()

        # exclude no-bite markers from hyperbola fit
        if CONFIG.FILTER == Filter.MANUAL:
            df_manual = self.manual_data
            types = df_manual["type"].to_numpy()
            fit_mask = (types != "Tooth.NO_BITE") & (types != "Tooth.CENTER_N")
            x = all_x[fit_mask]
            y = all_y[fit_mask]
        else:
            x = all_x
        ...
        coeff = np.matmul(
            np.linalg.inv(np.matmul(matrixT, matrix)),
            np.matmul(matrixT, np.ones(np.shape(matrix)[0])),
        )
        (A, B, C, D, E) = coeff
        x_inter = np.roots(
            [A, self.height * B + D, -1 + C * self.height**2 + E * self.height]
        )
        ...
        for i in range(len(hyperbola_fit[0])):
            projection = project_arclength(
                hyperbola_fit[0][i], hyperbola_fit[1][i], coeff
            )  # (x, y, normal, tagent)
            temp = []
        ...
        if CONFIG.FILTER == Filter.MANUAL:
            ...
            closest_ys = []
            side = [CONFIG.SQUARE for _ in range(len(all_x))]

            for tooth_ind in range(len(all_x)):
                proj_data = project_data_one(all_x[tooth_ind], all_y[tooth_ind], coeff)

                closest_x = proj_data[0]
                closest_y = proj_data[1]
                closest_proj_indecies.append(
                    np.argmin([abs(i - closest_x) for i in hyperbola_fit[0]])
                )
            ...
            n = df_manual.index[df_manual["type"] == "Tooth.CENTER_N"].to_numpy()
            assert len(t) + len(g) + len(n) < 2, (
                f"more than one center marker found in {self.img_name}.csv"
            )

            # if center tooth doesn't exist
            if len(t) + len(g) + len(n) == 0:
                ...
                elif df_manual["type"][center_ind] == "Tooth.NO_BITE":
                    df_manual["type"][center_ind] = "Tooth.CENTER_N"
            else:
                if (
                    len(t) > 0
                    and center_ind != t[0]
                    ...
                    print(
                        f"Alternative center index found: {center_ind}; please ensure the current center index is correct"
                    )
    ...
    def update_data_plot(self, mode: Match) -> None:
        ...
        if mode == Match.ONE_D:
            if self.image_proj is None:
                raise RuntimeError(
                    f"projected image for {self.img_name} is not found; did you run fit project first?"
                )
        ...
        for i in df["type"]:
            if i == "Tooth.TOOTH":
                ...
                type = np.append(type, Tooth.NO_BITE)
            elif i == "Tooth.CENTER_N":
                type = np.append(type, Tooth.CENTER_N)

        for i in range(len(x)):
            image = GUI.draw_tooth(
                image, int(x[i]), int(y[i]), w[i], h[i], type[i], Tooth.NO_BOX
            )
    ...
    def arclength_histogram(self, arclength_location: list[float]) -> list[float]:
        ...
        for location in range(len(arclength_location) - 1):
            distances.append(
                arclength_location[location + 1] - arclength_location[location]
            )
        ...
        suspect_distances = [
            ...
            if (
                distances[ind] <= CONFIG.ERROR_LOWER_B
                or distances[ind] >= CONFIG.ERROR_UPPER_B
            )
        ]
    ...
    def sum_dot_prod(self, coeff: tuple[float, float, float, float, float]) -> int:
        ...
        if len(x) < points:
            raise RuntimeError(
                f"Unable to perform {CONFIG.CROSS} crosses with {len(x)} points; {points} points needed"
            )
